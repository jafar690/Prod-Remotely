...
namespace Silgred.ScreenCast.Win.Services
{
    public class WinAudioCapturer : IAudioCapturer
    {
        ...
        private List<byte> TempBuffer { get; } = new List<byte>();
        ...
        private async void SendTempBuffer()
        {
            ...
            using (var ms1 = new MemoryStream())
            {
                ...
                using (var ms3 = new MemoryStream())
                {
                    ...
                    await conductor.CasterSocket.SendAudioSample(ms3.ToArray(),
                        Program.Conductor.Viewers.Keys.ToList());
                }
            }
        }
                ...
                Capturer.DataAvailable += (aud, args) =>
                {
                    try
                    {
                        if (args.BytesRecorded > 0)
                            lock (TempBuffer)
                            {
                                if (!SendTimer.IsRunning) SendTimer.Restart();
