...
namespace Silgred.ScreenCast.Win
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            try
            {
                ...
                if (Conductor.Mode == AppMode.Chat)
                    Services.GetRequiredService<ChatHostService>()
                        .StartChat(Conductor.RequesterID, Conductor.OrganizationName).Wait();
                else
                    StartScreenCasting();
            }
            catch (Exception ex)
