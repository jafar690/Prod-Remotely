...
namespace Silgred.ScreenCast.Core.Capture
{
    ...
    {
        public static ImageCodecInfo JpegEncoder { get; } = ImageCodecInfo.GetImageEncoders()
        ...
        {
            if (captureFullscreen) return new Rectangle(new Point(0, 0), currentFrame.Size);
            if (currentFrame.Height != previousFrame.Height || currentFrame.Width != previousFrame.Width)
                throw new Exception("Bitmaps are not of equal dimensions.");
            if (!Image.IsAlphaPixelFormat(currentFrame.PixelFormat) ||
                !Image.IsAlphaPixelFormat(previousFrame.PixelFormat) ||
                !Image.IsCanonicalPixelFormat(currentFrame.PixelFormat) ||
                !Image.IsCanonicalPixelFormat(previousFrame.PixelFormat))
            ...
            try
            {
                bd1 = previousFrame.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadOnly,
                    currentFrame.PixelFormat);
                bd2 = currentFrame.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadOnly,
                ...
                unsafe
                {
                    ...
                    for (var counter = 0; counter < totalSize - bytesPerPixel; counter += bytesPerPixel)
                    {
                        var data1 = scan1 + counter;
                        var data2 = scan2 + counter;
        ...
        {
            if (captureFullscreen) return (Bitmap) currentFrame.Clone();
            ...
            var bd1 = previousFrame.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadOnly,
                currentFrame.PixelFormat);
            var bd2 = currentFrame.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadOnly,
                previousFrame.PixelFormat);
            var bd3 = mergedFrame.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.WriteOnly,
                currentFrame.PixelFormat);
            ...
            var arraySize = Math.Abs(bd1.Stride) * currentFrame.Height;
            ...
            for (var counter = 0; counter < rgbValues2.Length - 4; counter += 4)
