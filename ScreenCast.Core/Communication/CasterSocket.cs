...
namespace Silgred.ScreenCast.Core.Communication
{
    public class CasterSocket : ICasterSocket
    {
        ...
        public async Task SendWebcamVideo(byte[] buffer, List<string> viewerIDs)
        {
            await Connection.SendAsync("SendWebcamVideo", buffer.Compress(), viewerIDs);
        }
        ...
        {
            cursor.ImageBytes = cursor.ImageBytes.Compress();
        ...
        public async Task SendIceCandidateToBrowser(string candidate, int sdpMlineIndex, string sdpMid,
            string viewerConnectionID)
        {
            await Connection.SendAsync("SendIceCandidateToBrowser", candidate, sdpMlineIndex, sdpMid,
                viewerConnectionID);
        }
        ...
        {
            await Connection.SendAsync("SendScreenCapture", captureBytes.Compress(), viewerID, left, top, width, height,
                captureTime);
        }
        ...
        public void ApplyConnectionHandlers()
        {
            ...
            Connection.On("MouseDown", (int button, double percentX, double percentY, string viewerID) =>
            {
                if (conductor.Viewers.TryGetValue(viewerID, out var viewer) && viewer.HasControl)
                {
                    ...
                    else if (button == 2) KeyboardMouseInput.SendRightMouseDown(percentX, percentY, viewer);
                }
            });
            ...
            Connection.On("MouseUp", (int button, double percentX, double percentY, string viewerID) =>
            {
                if (conductor.Viewers.TryGetValue(viewerID, out var viewer) && viewer.HasControl)
                {
                    ...
                    else if (button == 2) KeyboardMouseInput.SendRightMouseUp(percentX, percentY, viewer);
                }
            });
            ...
            Connection.On("ViewerDisconnected", async (string viewerID) =>
            {
                ...
                if (conductor.Viewers.TryGetValue(viewerID, out var viewer)) viewer.DisconnectRequested = true;
            ...
            {
                if (conductor.Viewers.TryGetValue(viewerID, out var viewer)) viewer.ImageQuality = qualityLevel;
            });

            Connection.On("AutoQualityAdjust", (bool isOn, string viewerID) =>
            {
                if (conductor.Viewers.TryGetValue(viewerID, out var viewer)) viewer.AutoAdjustQuality = isOn;
            });
        ...
        public async void ClipboardService_ClipboardTextChanged(object sender, string clipboardText)
        {
            var conductor = ServiceContainer.Instance.GetRequiredService<Conductor>();
            var viewerIDs = conductor.Viewers.Keys.ToList();
            if (viewerIDs.Any()) await SendClipboardText(clipboardText, viewerIDs);
        }
    }
}