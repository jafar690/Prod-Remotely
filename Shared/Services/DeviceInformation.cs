...
namespace Silgred.Shared.Services
{
    public class DeviceInformation
    {
        public static async Task<Device> Create(string deviceID, string orgID)
        {
            ...
            if (OSUtils.IsWindows)
                systemDrive = DriveInfo.GetDrives().FirstOrDefault(x =>
                    x.IsReady &&
                    x.RootDirectory.FullName.Contains(
                        Path.GetPathRoot(Environment.SystemDirectory ?? Environment.CurrentDirectory)));
            ...
            if (File.Exists("Remotely_Agent.dll"))
                device.AgentVersion = FileVersionInfo.GetVersionInfo("Remotely_Agent.dll")?.FileVersion?.Trim();
        ...
        public static string GetCurrentUser()
        {
            try
            {
                if (OSUtils.IsWindows) return Win32Interop.GetActiveSessions().LastOrDefault()?.Username;
