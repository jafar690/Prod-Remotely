    ...
    public class ScriptingController : ControllerBase
    {
        ...
        public async Task<ActionResult<CommandResult>> ExecuteCommand(string mode, string deviceID)
        {
            ...
            if (Request.HttpContext.User.Identity.IsAuthenticated)
            {
                ...
                userID = user.Id;
                if (!DataService.DoesUserHaveAccessToDevice(deviceID, user)) return Unauthorized();
            }

            Request.Headers.TryGetValue("OrganizationID", out var orgID);

            var connection = DeviceSocketHub.ServiceConnections.FirstOrDefault(x =>
                x.Value.OrganizationID == orgID &&
                x.Value.ID == deviceID);

            if (string.IsNullOrWhiteSpace(connection.Key)) return NotFound();
            ...
            var commandResult = new CommandResult
            {
                ...
                SenderUserID = userID,
                TargetDeviceIDs = new[] {deviceID},
                OrganizationID = orgID
            };
            DataService.AddOrUpdateCommandResult(commandResult);
            var requestID = Guid.NewGuid().ToString();
            await DeviceHub.Clients.Client(connection.Key).SendAsync("ExecuteCommandFromApi", mode, requestID, command,
                commandResult.ID, Guid.NewGuid().ToString());
            var success = await TaskHelper.DelayUntil(
                () => DeviceSocketHub.ApiScriptResults.TryGetValue(requestID, out _), TimeSpan.FromSeconds(30));
            if (!success) return commandResult;
