...
namespace Silgred.Server.Controllers
{
    ...
    {
        public OrganizationManagementController(DataService dataService, UserManager<RemotelyUser> userManager,
        ...
        public async Task<IActionResult> DeleteUser(string userID)
        {
            ...
            if (User.Identity.IsAuthenticated &&
                DataService.GetUserByName(User.Identity.Name).Id == userID)
                return BadRequest(
                    "You can't delete yourself here.  You must go to the Personal Data page to delete your own account.");
        ...
        public IActionResult DeviceGroupAddUser([FromBody] string userID, string groupID)
        {
            ...
            if (!result) return BadRequest(resultMessage);
        ...
        public async Task<IActionResult> GenerateResetUrl(string userID)
        {
            ...
            if (user.OrganizationID != orgID) return Unauthorized();
        ...
        public IActionResult Name([FromBody] string organizationName)
        {
            if (User.Identity.IsAuthenticated &&
                !DataService.GetUserByName(User.Identity.Name).IsAdministrator)
                return Unauthorized();
            if (organizationName.Length > 25) return BadRequest();
        ...
        public async Task<IActionResult> SendInvite([FromBody] Invite invite)
        {
            ...
            var emailResult = await EmailSender.SendEmailAsync(invite.InvitedUser,
            ...
            if (!emailResult) return Problem("There was an error sending the invitation email.");
