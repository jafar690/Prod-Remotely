...
namespace Silgred.Server.Services
{
    ...
    {
        Task<bool> SendEmailAsync(string email, string replyTo, string subject, string htmlMessage,
    ...
    public class EmailSenderEx : IEmailSenderEx
    {
        ...
        public Task<bool> SendEmailAsync(string email, string replyTo, string subject, string htmlMessage,
            string organizationID = null)
        {
            try
            {
                ...
                var from = new MailAddress(AppConfig.SmtpEmail, AppConfig.SmtpDisplayName, Encoding.UTF8);
