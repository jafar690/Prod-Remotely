...
using Silgred.Shared.ViewModels.Organization;
...
namespace Silgred.Server.Services
{
    public class DataService
    {
        ...
        public async Task AddAlert(AlertOptions alertOptions, string organizationID)
        {
            ...
            {
                var filteredUserIDs =
                    FilterUsersByDevicePermission(users.Select(x => x.Id), alertOptions.AlertDeviceID);
        ...
        public bool AddDeviceGroup(string orgID, DeviceGroup deviceGroup, out string deviceGroupID,
            ...
            else
            {
                ...
                if (!RemotelyContext.Organizations.Any(x => x.ID == device.OrganizationID))
                {
                    WriteEvent(new EventLog
                    {
                        ...
                        Message =
                            $"Unable to add device {device.DeviceName} because organization {device.OrganizationID} does not exist.",
        ...
        public void DeleteInvite(string orgID, string inviteID)
        {
            ...
            if (user != null && string.IsNullOrWhiteSpace(user.PasswordHash)) RemotelyContext.Remove(user);
            ...
            return RemotelyContext.Devices
                ...
                .Any(device => device.OrganizationID == remotelyUser.OrganizationID &&
                               ...
                               (
                                   ...
                                   device.DeviceGroup.PermissionLinks.Any(
                                       permission => permission.UserID == remotelyUser.Id
                                   )));
        }
        ...
        {
            var userPrompt = RemotelyContext.Users.FirstOrDefault(x => x.UserName == userName)?.UserOptions
                ?.ConsolePrompt;
        ...
        public void SetDeviceSetupOptions(string deviceID, DeviceSetupOptions options)
        {
            ...
            if (device != null)
            {
                if (!string.IsNullOrWhiteSpace(options.DeviceAlias)) device.Alias = options.DeviceAlias;
        ...
        public async Task UpdateServerAdmins(List<string> serverAdmins, string callerUserName)
        {
            ...
            foreach (var removeAdmin in removeAdmins) removeAdmin.IsServerAdmin = false;

            var newAdmins = RemotelyContext.Users.Where(user =>
                serverAdmins.Contains(user.UserName.Trim().ToLower()) &&
                !user.IsServerAdmin);

            foreach (var newAdmin in newAdmins) newAdmin.IsServerAdmin = true;
