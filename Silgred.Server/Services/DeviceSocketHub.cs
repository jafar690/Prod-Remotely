...
namespace Silgred.Server.Services
{
    public class DeviceSocketHub : Hub
    {
        ...
        public static ConcurrentDictionary<string, Device> ServiceConnections { get; } =
        ...
        {
            get => Context.Items["Device"] as Device;
            set => Context.Items["Device"] = value;
        }

        public Task BashResultViaAjax(string commandID)
        {
            var commandResult = DataService.GetCommandResult(commandID);
            return BrowserHub.Clients.Client(commandResult.SenderConnectionID)
                .SendAsync("BashResultViaAjax", commandID, Device.ID);
        }

        public Task Chat(string message, string senderConnectionID)
        {
            return BrowserHub.Clients.Client(senderConnectionID)
                .SendAsync("Chat", Device.ID, Device.DeviceName, message);
        }
        ...
        public Task CMDResultViaAjax(string commandID)
        {
            var commandResult = DataService.GetCommandResult(commandID);
            return BrowserHub.Clients.Client(commandResult.SenderConnectionID)
                .SendAsync("CMDResultViaAjax", commandID, Device.ID);
        }
            ...
            try
            {
                if (ServiceConnections.Any(x => x.Value.ID == device.ID))
                {
                    DataService.WriteEvent(new EventLog
                    {
                        ...
                        Message =
                            $"Device connection for {device?.DeviceName} was denied because it is already connected."
                    });
        ...
        {
            var commandResult = DataService.GetCommandResult(commandID);
            await BrowserHub.Clients.Client(commandResult.SenderConnectionID)
                .SendAsync("PSCoreResultViaAjax", commandID, Device.ID);
        }
        ...
        public Task WinPSResultViaAjax(string commandID)
        {
            var commandResult = DataService.GetCommandResult(commandID);
            return BrowserHub.Clients.Client(commandResult.SenderConnectionID)
                .SendAsync("WinPSResultViaAjax", commandID, Device.ID);
        }
    }
}