...
namespace Silgred.Server.Services
{
    public class BrowserSocketHub : Hub
    {
        ...
        public static ConcurrentDictionary<string, RemotelyUser> ConnectionIdToUserLookup { get; } =
            new ConcurrentDictionary<string, RemotelyUser>();
        ...
        {
            get => Context.Items["RemotelyUser"] as RemotelyUser;
            set => Context.Items["RemotelyUser"] = value;
        }
        ...
        public Task Chat(string message, string[] deviceIDs)
        {
            ...
            return DeviceHub.Clients.Clients(connections.Select(x => x.Key).ToList()).SendAsync("Chat",
                $"{RemotelyUser.UserName}: {message}", organizationName, Context.ConnectionId);
        }
        ...
        public Task DeployScript(string fileID, string mode, string[] deviceIDs)
        {
            ...
            foreach (var connection in connections)
                DeviceHub.Clients.Client(connection.Key).SendAsync("DeployScript", mode, fileID, commandResult.ID,
        ...
        public Task ExecuteCommandOnClient(string mode, string command, string[] deviceIDs)
        {
            ...
            Clients.Caller.SendAsync("CommandResultCreated", commandResult);
            foreach (var connection in connections)
                DeviceHub.Clients.Client(connection.Key).SendAsync("ExecuteCommand", mode, command, commandResult.ID,
                    Context.ConnectionId);
        ...
        public override async Task OnConnectedAsync()
        {
            ...
            if (await IsConnectionValid() == false) return;
        ...
        public Task RemoteControl(string deviceID)
        {
            ...
            {
                var currentUsers =
                    RCDeviceSocketHub.SessionInfoList.Count(x => x.Value.OrganizationID == RemotelyUser.OrganizationID);
                if (currentUsers >= AppConfig.RemoteControlSessionLimit)
                    return Clients.Caller.SendAsync("DisplayMessage",
                ...
                return DeviceHub.Clients.Client(targetDevice.Key)
                    .SendAsync("RemoteControl", Context.ConnectionId, targetDevice.Key);
            }

            DataService.WriteEvent(
                $"Remote control attempted by unauthorized user.  Device ID: {deviceID}.  User Name: {RemotelyUser.UserName}.",
                EventType.Warning, targetDevice.Value.OrganizationID);
            return Task.CompletedTask;
        }
        ...
        public Task UploadFiles(List<string> fileIDs, string transferID, string[] deviceIDs)
        {
            DataService.WriteEvent(new EventLog
            {
                ...
                Message =
                    $"File transfer started by {RemotelyUser.UserName}.  File transfer IDs: {string.Join(", ", fileIDs)}.",
            ...
            var connections = GetActiveClientConnections(deviceIDs);
            foreach (var connection in connections)
                DeviceHub.Clients.Client(connection.Key)
                    .SendAsync("UploadFiles", transferID, fileIDs, Context.ConnectionId);
            return Task.CompletedTask;
        }
